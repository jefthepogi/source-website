...
import officerData from '../assets/officers.json';
...
function OfficersPage() {
  ...
  const categorizedOfficers = officerData.map(category => ({
    ...
    officers: category.officers.map(officer => ({
      ...officer,
      image: requireImage(officer.image) // Ensure `img` is correct here
    }))
  }));
  ...
  return (
    <>
      {/* Header Section */}
      <div className="bg-[#087830] text-white padding py-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-5xl font-bold">Meet the Officers</h1>
